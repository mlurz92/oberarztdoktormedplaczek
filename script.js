...
function renderSongs() {
  ...
  state.filteredSongs.forEach((song, index) => {
    ...
    card.querySelector(".song-card__lyrics-preview").textContent = song.lyrics;
    card.querySelector("details pre").textContent = song.lyrics;
...
function filterSongs() {
  ...
  state.filteredSongs = state.songs.filter((song) => {
    const searchContent = `${song.title} ${song.styles} ${song.lyrics}`.toLowerCase();
...
function highlightSong(index) {
  ...
  selectors.heroHook.textContent = song.hook || "";
